...
class ViewCreator():
    ...
    def combobox(
        ...
        if state >= 0:
            v = selection[state]
        hCombo = wx.ComboBox(parent, wx.ID_ANY, value=v, choices=selection,
                             style=wx.BORDER_RAISED | style, name=text, size=(x, -1))
        ...
        if isinstance(text, str):
            ...
            for s in text:
                if len(radios) == 0:  # 最初の１つのみ追加のスタイルが必要
                    hRadio = wx.RadioButton(
                        hPanel, id=wx.ID_ANY, label=s, style=wx.RB_GROUP | style, name=s)
                else:
                    hRadio = wx.RadioButton(
                        hPanel, id=wx.ID_ANY, label=s, style=style, name=s)
                hRadio.Bind(wx.EVT_RADIOBUTTON, event)
                self._setFace(hRadio, mode=SKIP_COLOUR)
                Add(hSizer, hRadio)
    ...
    def listbox(self, text, choices=[], event=None, state=-1, style=0, size=(-1, -1),
                sizerFlag=wx.ALL, proportion=0, margin=5, textLayout=wx.DEFAULT):
        hStaticText, sizer, parent = self._addDescriptionText(
            text, textLayout, sizerFlag, proportion, margin)
    ...
    def inputbox(
        ...
        hTextCtrl = TextCtrl(parent, wx.ID_ANY, size=(
            x, -1), name=text, value=defaultValue, style=style | wx.BORDER_RAISED)
    ...
    def slider(
        ...
        hSlider = wx.Slider(
    parent, wx.ID_ANY, size=(
         x, -1), value=defaultValue, minValue=min, maxValue=max, style=style)
