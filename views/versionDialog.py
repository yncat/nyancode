...
class dialog(baseDialog.BaseDialog):
    ...
    def InstallControls(self):
        """いろんなwidgetを設置する。"""
        versionCreator = ViewCreator.ViewCreator(
            self.viewMode,
            self.panel,
            self.sizer,
            wx.VERTICAL,
            ...
            margin=20)
        textList = []
        textList.append(str(constants.APP_FULL_NAME) +
                        " (" + constants.APP_NAME + ")")
        ...
        try:
            textList.append(
                _("アップデータバージョン") +
                ": " +
                update.getUpdaterVersion()[1])
        except BaseException:
            pass
        textList.append(_("ライセンス") + ": " + constants.APP_LICENSE)
        textList.append(
            _("開発元") + ": %s - %s" %
            (constants.APP_DEVELOPERS, constants.APP_DEVELOPERS_URL))
        ...
        self.info, dummy = versionCreator.inputbox("", defaultValue="\r\n".join(
            textList), style=wx.TE_MULTILINE | wx.TE_READONLY | wx.TE_NO_VSCROLL | wx.BORDER_RAISED, sizerFlag=wx.EXPAND, x=750, textLayout=None)
        f = self.info.GetFont()
        ...
        # フッター
        footerCreator = ViewCreator.ViewCreator(
            self.viewMode,
            self.panel,
            self.sizer,
