...
class updateDialog(BaseDialog):
    ...
    def InstallControls(self):
        ...
        if self.info["code"] == 200:
            self.infoStatic = self.creator.staticText(
                _("version %s にアップデートできます。") %
                (self.info["update_version"]))
            self.gauge, self.gaugeStatic = self.creator.gauge(
                _("進行状況"), x=600, style=wx.TOP, margin=5, textLayout=None)
            self.gauge.Disable()
            self.infoEdit, self.infoEditStatic = self.creator.inputbox(_("version %s アップデート情報") % (
                self.info["update_version"]), defaultValue=self.info["update_description"], x=600, style=wx.TE_MULTILINE | wx.TE_READONLY | wx.TE_DONTWRAP)
            ...
            self.infoEdit, self.infoEditStatic = self.creator.inputbox(
                _("緊急のお知らせがあります。"), defaultValue=self.info["info_description"], style=wx.TE_MULTILINE | wx.TE_READONLY | wx.TE_DONTWRAP)
    ...
    def updater_notFound(self):
        simpleDialog.dialog(
            _("エラー"),
            _("updater.exeが見つかりませんでした。誤って削除したかなどをご確認ください。"))
        self.end()
