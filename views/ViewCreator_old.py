...
class ViewCreator():
    ...
    def __init__(
        ...
        if orient == FlexGridSizer:
            ...
            self.sizer = self.BoxSizer(
                parentSizer, orient, label, space, style)
    ...
    def okbutton(
            ...
            text,
            event=None,
            sizerFlag=wx.ALIGN_BOTTOM | wx.ALIGN_RIGHT | wx.ALL,
            proportion=0):
        hButton = wx.Button(
            self.parent,
            wx.ID_OK,
    ...
    def staticText(
            ...
            sizerFlag=0,
            proportion=0):
        return _staticText(self, title, text, x, -1,
                           style, sizerFlag, proportion)
    ...
    def _staticText(self, title, text, x=, y=, style, sizerFlag, proportion):
        hStatic = wx.StaticText(
            self.parent, -1, label=text, name=title, size=(x, -1), style=style)
    ...
    def comboEdit(self, text, selection, x=-1, event=None, defaultValue=""):
        ...
        hCombo = wx.ComboBox(self.parent, wx.ID_ANY, value=defaultValue,
                             choices=selection, style=wx.CB_DROPDOWN, name=text, size=(x, -1))
        hCombo.Bind(wx.EVT_TEXT, event)
    ...
    def checkbox(self, text, event, state=False):
        ...
        if (isinstance(text, str)):  # 単純に一つを作成
            ...
            Add(self.sizer, hPanel, 0, wx.BOTTOM |
                wx.LEFT | wx.RIGHT, self.space)
            ...
            for i, s in enumerate(text):
                if (state is None):
                    ...
                    hCheckBox = wx.CheckBox(
                        ...
                        label=s,
                        name=s,
                        style=wx.CHK_ALLOW_3RD_STATE_FOR_USER | wx.CHK_3STATE)
    ...
    def tabCtrl(
            ...
            title,
            event=None,
            style=wx.NB_NOPAGETHEME | wx.NB_MULTILINE,
            proportion=0,
            sizerFlag=0):
    ...
    def inputbox(self, text, x=0, defaultValue="", style=0):
        ...
        hTextCtrl = TextCtrl(self.parent, -1, size=(x, -1),
                             name=text, value=defaultValue, style=style)
        self.SetFace(hTextCtrl)
        if x == -1:  # 幅を拡張
            if (self.sizer.__class__ == wx.BoxSizer or self.sizer.__class__ ==
                    wx.StaticBoxSizer) and self.sizer.GetOrientation() == wx.HORIZONTAL:
    ...
    def webView(self):
        wx.html2.WebView.MSWSetEmulationLevel(
            wx.html2.WEBVIEWIE_EMU_IE11_FORCE)
