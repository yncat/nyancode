...
if len(sys.argv) == 2 and sys.argv[1] == "--appveyor":
    appveyor = True
print("Starting build for %s(appveyor mode=%s)" %
      (constants.APP_NAME, appveyor))
...
package_path = os.path.join(
    "dist", os.path.splitext(
        os.path.basename(
            constants.STARTUP_FILE))[0])
...
version_text = version_text.replace(
    "%REGAL_COPYRIGHT%",
    constants.APP_COPYRIGHT_MESSAGE)
original_file_name = os.path.splitext(
    os.path.basename(constants.STARTUP_FILE))[0] + ".exe"
...
if constants.APP_ICON is None:
    ...
    runcmd(
        "%s --windowed --log-level=ERROR --version-file=version.txt --icon=%s %s" %
        (pyinstaller_path, constants.APP_ICON, constants.STARTUP_FILE))
shutil.copytree(
    "locale\\", os.path.join(
        package_path, "locale"), ignore=shutil.ignore_patterns(
            "*.po", "*.pot", "*.po~"))
...
if build_filename == "snapshot":
    ...
    if constants.BASE_PACKAGE_URL is not None:
        ...
        archiver = diff_archiver.DiffArchiver(
            constants.BASE_PACKAGE_URL,
            archive_name,
            patch_name,
