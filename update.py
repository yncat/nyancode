    ...
    def update(self, auto=False):
        ...
        if not hasattr(sys, "frozen"):
            ...
            if not auto:
                simpleDialog.winDialog(
                    _("アップデート"), _("このバージョンではアップデートを使用できません。"))
            return
        # アップデータチェック
        updaterCheck = False
        try:
        ...
        try:
            response = requests.get(
                constants.UPDATE_URL, params, timeout=timeout)
        ...
        if code == errorCodes.UPDATER_LATEST:
            ...
            if not auto:
                simpleDialog.winDialog(
                    _("アップデート"), _("現在のバージョンが最新です。アップデートの必要はありません。"))
            return
    ...
    def runUpdate(self):
        ...
        if os.path.exists("updater.exe"):
            ...
            subprocess.Popen(
                ("updater.exe",
                 sys.argv[0],
                    constants.UPDATER_WAKE_WORD,
                    self._file_name,
                    self.info["updater_hash"],
                    str(pid)))
        else:
